...
const MAX_RETRIES = 5;
export async function shortenUrl(original) {
...
export async function getByShortCodeAndIncrement(shortCode) {
    // Atomically find and increment clicks
    const doc = await UrlModel.findOneAndUpdate({ shortCode }, { $inc: { clicks: 1 } }, { new: true }).exec();
    return doc;
}
